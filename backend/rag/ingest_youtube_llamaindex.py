...
# YOUTUBE_API_KEY = os.environ["YOUTUBE_API_KEY"]  # moved out of source — see note below
...
if not YOUTUBE_API_KEY:
    raise ValueError(
        ...
        "Set it as an environment variable or in a .env file before running this script."
    )
...
SUNDRAM_RECORDINGS = [
...

    {
        "title": "Math Session",
        "topic": "mathematics",
        "keywords": ["mathematics", "number sense"],
        "date": "15/08/2020",
...
# Determine embedding dimension from the model itself
...
youtube_index = VectorStoreIndex.from_documents(
    documents,
    storage_context=storage_context,
    embed_model=embed_model
)

# Persist both the FAISS index and the LlamaIndex docstore/index metadata
youtube_index.storage_context.persist(persist_dir=YOUTUBE_FAISS_PATH)

print("✅ YouTube FAISS created")
print("📦 Files created in:", YOUTUBE_FAISS_PATH)