...
# LlamaIndex uses a global Settings object for the embed model
# when loading/querying an index unless you pass embed_model explicitly.
...
def load_faiss_index(persist_dir: str):
...
books_index = load_faiss_index(str(BASE_DIR / "vectorstore" / "books"))
feedback_index = load_faiss_index(str(BASE_DIR / "vectorstore" / "feedback"))

web_index = load_faiss_index(str(BASE_DIR / "vectorstore" / "web"))

youtube_index = load_faiss_index(str(BASE_DIR / "vectorstore" / "youtube"))
...
def search_index(index, query, k=4):
...
def search_youtube_ranked(index, query, k=5):
...
def get_top_youtube_videos(index, query):

    results = search_youtube_ranked(index, query)
